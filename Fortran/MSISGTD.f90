      ...
      subroutine GTD7Tropo(IDY,ALT,GLAT,GLONG,STL,Density,Temp,Press)
      !     returns Density , Temperature and pressure for troposphere

            DIMENSION D(9),T(2), AP(10), SW(25)
             REAL ALT, GLAT, GLONG, STL, SEC, P, KALT
             REAL F107A, F107,Density,Temp,Press
             INTEGER IDY
            ...
            call GTD7AP(IDY,SEC,KALT,GLAT,GLONG,STL,F107A,F107,AP,MASS,D,T,P,SW)
            Density = D(6)
            Temp =  T(2) - 273.15
            Press = P/100.0
      ...
      SUBROUTINE GTD7AP(DY,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,MASS,D,T,P,SW)
      !     Calculate pressure from output data
      INTEGER  DY, MASS
      REAL ALT, GLAT,GLONG,STL,F107A,F107,SEC,D,T,P,SW, AP
      REAL BM,XN
      DIMENSION D(9),T(2),SW(25), AP(10)
...
!C      DATA SW/25*1./
      ...
      CALL GTD7D(DY,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,MASS,D,T)

      XN=D(1)+D(2)+D(3)+D(4)+D(5)+D(7)+D(8)+D(9)
      P=BM*XN*T(2)
      P= P*1.E-4
...
!C
      INTEGER IYD, MASS, MN3,MN2, I, MSS, MSSL
      REAL SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,D,T
      REAL XLAT,ZMIX,ALAST
      REAL DS,TS,ZN3,ZN2,SV
      DIMENSION D(9),T(2),AP(10),DS(9),TS(2)
     ...
     & ,G0,RL,DD,DB14,TR12
      ...
      COMMON/PARM7/PT(150),PD(150,9),PS(150),PDL(25,2),PTL(100,4),PMA(100,10),SAM(100)
...
!C        T(2) - TEMPERATURE AT ALT
!C
!C     DIMENSION D(9),T(2),AP(7),DS(9),TS(2)
      ! INTEGER IYD
      ! REAL SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,D,T,MASS
      DIMENSION D(9),T(2),AP(10)
      COMMON/METSEL/IMR
...
!C
      INTEGER IYD, IDAY, L
      REAL SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,D,T,PRESS
      REAL PL, RGAS,BM, TEST, LTEST, ZI ,CL , CL2, CA, Z, CD
      REAL P, XM, XN, G , GSURF, SH, DIFF
      COMMON/PARMB/GSURF,RE
      COMMON/METSEL/IMR
      DIMENSION D(9),T(2),AP(10)
      SAVE
...
!C       Return 0 if unchanged and 1 if changed
      INTEGER IYD, IC, IYDL, I, VTST7
      REAL SEC,GLAT,GLONG,STL,F107A,F107,AP
      REAL SECL, GLATL, GLL, STLL, FAL, FL,APL, SWL, SWCL
...
!C        T(2) - TEMPERATURE AT ALT
!C
      INTEGER IYD,YRD, MASS, MT, MN1
       REAL SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,D,T
       REAL ALTL,ZN1,DGTR,DR,ALAST, ALPHA, V2, DAY
       REAL ZA, TLB, TINF, G0, TN1,Z0,T0,XMM,G28
       REAL DD, ZH28, ZHM28,ZHF
      DIMENSION ZN1(5),ALPHA(9)
      COMMON/GTS3C/TLB,S,DB04,DB16,DB28,DB32,DB40,DB48,DB01,ZA,T0,Z0 &
     & ,G0,RL,DD,DB14,TR12
      COMMON/MESO7/TN1(5),TN2(4),TN3(5),TGN1(2),TGN2(2),TGN3(2)
      DIMENSION D(9),T(2),MT(11),AP(10),ALTL(8)
      COMMON/LOWER7/PTM(10),PDM(10,8)
      COMMON/PARM7/PT(150),PD(150,9),PS(150),PDL(25,2),PTL(100,4),PMA(100,10),SAM(100)
      COMMON/CSW/SW(25),ISW,SWC(25)
...
!C       N2 variation factor at Zlb
      ...
      DAY=AMOD(REAL(YRD),1000.)
!C        VARIATION OF TURBOPAUSE HEIGHT
      ZHF=PDL(25,2)*(1.+SW(5)*PDL(25,1)*SIN(DGTR*GLAT)*COS(DR*(DAY-PT(14))))
      YRD=IYD
      T(1)=TINF
      XMM=PDM(5,3)
      Z=ALT
...
!C      Diffusive density at Alt
      D(3)=DENSU(Z,DB28,TINF,TLB, 28.,ALPHA(3),T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
      DD=D(3)
!C      Turbopause
      ZH28=PDM(3,3)*ZHF
...
!C      Mixed density at Zlb
      B28=DENSU(ZH28,DB28,TINF,TLB,XMD,ALPHA(3)-1.,TZ,PTM(6),S,MN1,ZN1,TN1,TGN1)
      IF(Z.GT.ALTL(3).OR.SW(15).EQ.0.) GO TO 17
!C      Mixed density at Alt
      DM28=DENSU(Z,B28,TINF,TLB,XMM,ALPHA(3),TZ,PTM(6),S,MN1,ZN1,TN1,TGN1)
...
!C      Diffusive density at Alt
      D(1)=DENSU(Z,DB04,TINF,TLB, 4.,ALPHA(1),T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
      DD=D(1)
      IF(Z.GT.ALTL(1).OR.SW(15).EQ.0.) GO TO 24
!C      Turbopause
      ZH04=PDM(3,1)
!C      Mixed density at Zlb
      B04=DENSU(ZH04,DB04,TINF,TLB,4.-XMM,ALPHA(1)-1.,T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
!C      Mixed density at Alt
...
!C       Diffusive density at Alt
      D(2)=DENSU(Z,DB16,TINF,TLB, 16.,ALPHA(2),T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
      DD=D(2)
...
!C       Diffusive density at Alt
      D(4)=DENSU(Z,DB32,TINF,TLB, 32.,ALPHA(4),T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
      IF(MASS.EQ.49) THEN
         DD=DD+2.*D(4)
      ELSE
         DD=D(4)
...
!C       Turbopause
      ZH32=PDM(3,4)
!C      Mixed density at Zlb
      B32=DENSU(ZH32,DB32,TINF,TLB,32.-XMM,ALPHA(4)-1.,T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
!C      Mixed density at Alt
...
!C       Turbopause
      ZH40=PDM(3,5)
!C      Mixed density at Zlb
      B40=DENSU(ZH40,DB40,TINF,TLB,40.-XMM,ALPHA(5)-1.,T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
!C      Mixed density at Alt
...
!C       Turbopause
      ZH01=PDM(3,6)
!C      Mixed density at Zlb
      B01=DENSU(ZH01,DB01,TINF,TLB,1.-XMM,ALPHA(7)-1.,T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
!C      Mixed density at Alt
...
!C       Diffusive density at Alt
      D(8)=DENSU(Z,DB14,TINF,TLB,14.,ALPHA(8),T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
      DD=D(8)
      IF(Z.GT.ALTL(8).OR.SW(15).EQ.0.) GO TO 49
!C       Turbopause
      ZH14=PDM(3,7)
!C      Mixed density at Zlb
      B14=DENSU(ZH14,DB14,TINF,TLB,14.-XMM,ALPHA(8)-1.,T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
!C      Mixed density at Alt
...
!C
      ...
      DD=DENSU(Z,DB16H,THO,THO,16.,ALPHA(9),T2,PTM(6),S,MN1,ZN1,TN1,TGN1)
      ZSHT=PDM(6,8)
      ZMHO=PDM(5,8)
...
!C       TOTAL MASS DENSITY
!C
      D(6) = 1.66E-24*(4.*D(1)+16.*D(2)+28.*D(3)+32.*D(4)+40.*D(5)+D(7)+14.*D(8))
      DB48=1.66E-24*(4.*DB04+16.*DB16+28.*DB28+32.*DB32+40.*DB40+DB01+14.*DB14)
      GO TO 90
...
!C       Upper Thermosphere Parameters
      INTEGER YRD,NSW
       REAL SEC,LAT,LONG,TLOC,F107A,F107,AP,P
       REAL SV,DGTR,DR,XL,TLL,SW9,DAYL,P14,P18,P32
       REAL HR,SR,P39
       REAL A,EX,G0,SUMEX,SG0
      DIMENSION P(140),SV(25),AP(10)
...
!C       Eq. A24a
     ...
     & +(G0(AP(6))*EX**4+G0(AP(7))*EX**12)*(1.-EX**8)/(1.-EX)))/SUMEX(EX)
      IF(ISW.NE.64999) CALL TSELEC(SV)
      DO 10 J=1,14
       T(J)=0
   10 CONTINUE
...
!C        TERDIURNAL
      ...
      T(14) = F2*&
     & ((P(40)*PLG(4,4)+(P(94)*PLG(5,4)+P(47)*PLG(7,4))*CD14*SWC(5))*S3TLOC &
...
!C      VERSION OF GLOBE FOR LOWER ATMOSPHERE 10/26/99
      REAL P, T ,DR, DGTR,PSET,DAYL,P32,P18,P14,P39
      REAL cd32, cd18, cd14, cd39, T81, T82
      COMMON/LPOLY/PLG(9,4),CTLOC,STLOC,C2TLOC,S2TLOC,C3TLOC,S3TLOC,&
...
!C       MAGNETIC ACTIVITY
      ...
      IF(SW(9).EQ.-1) T(9)=(P(51)*APT(1)+P(97)*PLG(3,1)*APT(1)*SWC(2))
   40 CONTINUE
...
!C       New lower thermo polynomial 10/30/89
      INTEGER MN1,MN,K,IERR,IFUN,N,J
      REAL ALT,DLB,TINF,TLB,XM,ALPHA,TZ,ZLB,S2,ZN1,TN1,TGN1
      REAL YD2,YD1,X,Y,ZA,Z,ZG2,TT,TA
      REAL ZETA,DTA,Z1,Z2,T1,T2,ZG,ZGDIF
      REAL GAMMA,GAMM,DENSA,YI,EXPL,GLB
      REAL XS,YS,Y2OUT
...
!C       Calculate Temperature and Density Profiles for lower atmos.
       INTEGER MN3, MN2, MN, K, N, J
       REAL ALT,D0,XM,TZ,ZN3,TN3,TGN3,ZN2,TN2,TGN2
       REAL Z,Z1,Z2,ZZ,ZG,ZGDIF,ZL,ZETA
       REAL XS,YS,Y2OUT
       REAL X,Y,YI,YD1,YD2
       REAL GAMM,GLB,EXPL,IERR,IFUN
...
!C        Y2: OUTPUT ARRAY OF SECOND DERIVATIVES
      INTEGER NMAX, N, I, K
      REAL X,Y,YP1,YPN,Y2,P,SIG,U,QN,UN
      PARAMETER (NMAX=100)
...
!C        Y: OUTPUT VALUE
      INTEGER N,KHI,KLO
      REAL XA,YA,Y2A,X,A,A2,B,B2,XX,H
      REAL YI
      DIMENSION XA(N),YA(N),Y2A(N)
      SAVE
      YI=0
...
! *         H2 - transition scale length #2 ?
      ...
      REAL ALT,R,H1,ZH,H2,E1,E2,EX1,EX2
      E1=(ALT-ZH)/H1
      E2=(ALT-ZH)/H2
...
!C LATITUDE:-90 TO 90. LONGITUDE:0 TO 360 EAST.
      ! INTEGER ART
      ! REAL XLG,BG,XLM,BM,CBG,CBM,CI,CLG,CLM,FAKTOR,SBG,SBM
      ! REAL SI, SLG, SLM, YLG, ZPI
      FAKTOR=.0174532925
      ZPI=FAKTOR*360.
      CBG=11.4*FAKTOR
      CI=COS(CBG)
